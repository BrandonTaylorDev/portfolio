	...
	<div class="relative z-10 mx-auto max-w-6xl px-6 py-12">
		...
		<div class="grid gap-8 md:grid-cols-2">
			{#each categories as category}
				<div
					class="group relative overflow-hidden rounded-lg border-2 border-zinc-700 bg-zinc-800/50 p-8 shadow-lg backdrop-blur-sm transition-all duration-300 hover:border-teal-500 hover:shadow-2xl"
				>
					<!-- Icon and Title Row -->
					...
					<div class="flex flex-wrap gap-2">
						{#each category.items as tool}
							<span
								class="rounded-md border border-zinc-700 bg-zinc-900/80 px-3 py-1.5 text-sm text-zinc-300 transition-all duration-200 hover:border-teal-500 hover:bg-zinc-800 hover:text-teal-400 hover:shadow-md"
							>
								{tool}
							</span>
						{/each}
